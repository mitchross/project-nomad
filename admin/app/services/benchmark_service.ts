...
// AI_BENCHMARK_MODEL env override is for non-Ollama deployments (e.g. vLLM
// serving a fixed model) — an overridden run's score is NOT comparable to the
// reference baseline; leave unset on Ollama for leaderboard-comparable results.
const AI_BENCHMARK_MODEL = process.env.AI_BENCHMARK_MODEL || 'llama3.1:8b'
...
export class BenchmarkService {
  ...
  private async _runSystemBenchmarks(multiThreads: number): Promise<SystemBenchmarkOutput> {
    // In Kubernetes mode, Docker is unavailable so sysbench-in-Docker cannot run.
  ...
  private async _runAIBenchmark(): Promise<AIScores> {
    ...
    try {
      ...
      // Non-Ollama backends (vLLM, llama.cpp via LLM_PROVIDER=openai) don't expose
      // the native /api/generate metrics this flow depends on — use the
      // OpenAI-compatible wall-clock path instead.
      const providerService = new (await import('./ollama_service.js')).OllamaService()
      if (!providerService.provider.supportsNativeBenchmark()) {
        return await this._runOpenAIAIBenchmark(providerService)
      }

      // getServiceURL resolves the UI-configured remote URL and (in Kubernetes
      // mode) LLM_HOST/OLLAMA_HOST; the env fallback covers Docker mode with an
      // env-configured remote Ollama.
      const { default: env } = await import('#start/env')
      const ollamaAPIURL =
        (await this.dockerService.getServiceURL(SERVICE_NAMES.OLLAMA)) ||
        env.get('LLM_HOST') ||
        env.get('OLLAMA_HOST')
      if (!ollamaAPIURL) {
  ...
  /**
   * OpenAI-compatible AI benchmark (vLLM, llama.cpp, ...): wall-clock timing via
   * the provider's chat API. Token counts are estimated from the response text, so
   * scores are indicative for the user's own hardware comparisons — NOT comparable
   * to the Ollama-native reference baseline (which measures eval_count/duration on
   * a fixed reference model).
   */
  private async _runOpenAIAIBenchmark(
    ollamaService: InstanceType<typeof import('./ollama_service.js').OllamaService>
  ): Promise<AIScores> {
    // Use the configured benchmark model if the backend serves it, else the first available
